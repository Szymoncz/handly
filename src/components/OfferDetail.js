...
export default function OfferDetail() {
  ...
  const normalizeId = (val) => {
    ...
    if (typeof val === "string") {
      // Handles both URLs and string-numbers
  ...
  const isMyOffer = Boolean(
    creatorId && loggedInUserId && creatorId === loggedInUserId,
  );
  ...
  return (
    <>
      {/* DEBUG HEADER - REMOVE THIS AFTER IT WORKS */}
      <div
        style={{
          background: "#333",
          color: "#0f0",
      ...
      >
        Debug: Twój ID: {loggedInUserId || "BRAK"} | ID Autora:{" "}
        {creatorId || "BRAK"} | Zgodność: {isMyOffer ? "TAK" : "NIE"}
      </div>
      ...
      <style>{`
        ...
        .delete-btn { background-color: #ef4444; border: none; color: white; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-weight: bold; }
      ...
      <div className="detail-container">
        ...
        <div className="image-section">
          {images[0] ? (
            <img src={images[0].image} className="main-image" alt="Oferta" />
          ) : (
